...
This chapter describes the process of conceptualizing and developing the project Tiny Trainable Instruments. It is an antidisciplinary thesis living at the interstitial spaces between the fields of \acrlong{AI}, electrical engineering, computer science, media arts, music, and pedagogy, among others.

This project is a hands-on invitation to ethical creative \acrshort{ML} and instrument making with microcontrollers. Also, a critical response to the current state of industrial \acrshort{ML} with its harmful algorithmic bias and oppressive facial recognition technologies.

It is a wrapper that incorporates the recent developments of tiny \acrshort{ML} and adapts them for multimedia art, with the hope that artists and enthusiasts of different disciplines can create ethical and privacy-respecting instruments powered by \acrshort{AI}. 

This project lays down the complete infastructure, for people to learn how to use microcontrollers and sensors, in order to create their own databases, and then train and deploy \acrshort{ML} algorithms that control multiple artistic actuators.

With the infrastructure laid by Tiny Trainable Instruments, they now can create devices that can recognize their voice and words, their movement and physical gestures in space, and colors.

lays the basic infrastructure

This framework of software and hardware 

 foster artistic experimentation, 

In order to promote 

this field still face many barriers of entry, including high costs and specialized hardware.

This project is about respecting and understanding how to make your own databases and program your own \acrshort{ML} models for artistic purposes, while respecting your privacy, building your own small databases and deploying to ecosystems with no backdoors or restrictions by corporations or governments.

but also that make you feel safe, which I think is a requisite for being able to create.

Tiny Trainable Instruments is a platform for critical, safe, and artistic creation, by using microcontrollers as a low cost and low level alternative to other computational systems with more restrictive barriers, such as high costs, geographical regulations, required certifications for developers, among others.

With this thesis I hope that you train \acrshort{ML} models that not only react to your own custom database, and understand your accent, 
...
EXPLAIN HOW THIS IS AN ART PROJECT, WHY THE POLITICAL CONTEXT MAKES SENSE FOR ME AND EVRYBODY, 
...
The speech database we built is the most compromising one, since it is identifiable, and it could even be used for cloning your voice with deepfake techniques. So this is the one I encourage people to be more mindful about when publishing their custom databases.
...
I see the servo motor as a very versatile output, which can be thought of as a generator of movement. So your Tiny Trainable Instrument can dance in space, or point at different places. For this library, I programmed support for moving the servo at different tempos, even with some random noise for hiccupy movement, so that the servo motor can be thought of as a robotic arm for drumming or tapping on different surfaces for music. In this image, you can see an example of this with some tape and wire for extending the arm of the servo motor.
