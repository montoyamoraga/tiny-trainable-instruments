...
In 2015, Critter \& Guitari released the Organelle, a sound computer running the Linux operating system and the Pure Data software as the programmable interface between the hardware and the sound engine. Even though aany sound you can do on an Organelle you could do on your laptop using Pure Data, I think the focus on this computer on making sound it's amazing for musicians. You don't need to be a programmer to use it, it ships with presets, and there is a community of people sharing software for others to use, that you can easily install.
...
After the release of the Organelle, this company released the ETC computer for visuals. It also runs the Linux operating system, and it uses the pygame library for generating graphics. You can write your own scripts on Python to modify its behavior. The ETC was discontinued and replace with a compatible and similar instrument, the EYESY, which has a new mode for running openFrameworks.
