...
The color input allows artists to build a Tiny trainable instrument that can detect the color of objects near to the microcontroller. It accomplishes this feat by relying on 2 software libraries: Arduino{\_}APDS9960 for the sensor, and Arduino{\_}KNN for \acrshort{ML}.

I recommend that this is the first input that people try, since it is the easiest to implement, and the fastest to use.



intended as the simplest and fastest input, and it is recommended to start building with this one.

In terms of software, it relies on 2 libraries: 



For detecting color, we are using 2 different sensors of the Arduino which is able to 
