...
The core componentes of this thesis are:

\begin{enumerate}
  \item Repository with TinyTrainable Arduino software library
  \item Code and guides for \acrshort{ML} and custom databases
  \item Workshop, documentation, and educational material
\end{enumerate}

In this chapter I will explain both the process and the result of each component, but first I will define the words that make up the title of this project.

\section{Definition of Tiny trainable instruments}

\subsection{Definition of tiny}

By \emph{tiny} I mean lightweight handheld instruments, that you can fit on a backpack, or take for a walk. This is inspired on my personal practice, for years I have accumulated many small tools for arts that I combine for performing on gigs or while traveling, including small electronic synthesizers, sound mixers, battery-powered speakers. Here is an example of a handheld synthesizer that I use for field recordings and sampling, with a battery pack.
...
\begin{figure}[ht]
  ...
  \caption{Sampler synthesizer with microphone and portable battery}
  \caption*{Picture taken by myself}
  \label{fig:critter-and-guitari-kaleidoloop-battery}
\end{figure}

Tiny is also a nod to the emerging field of tiny \acrshort{ML}, and the community behind this movement, including most importantly the tinyMLx professional certificate\cite{website-edx-harvardx-tinymlx-professional-certificate} offered by HarvardX, that I completed while working on this project.

\subsection{Definition of trainable}

By \emph{trainable} I mean a computational device that can learn by examples. This name comes from the \acrshort{ML} lexicon, where training is a process where an algorithm  iteratively finds the numerical values of all parameters (weights, biases) of a \acrshort{ML} model.

The possibility of training the instrument   really rewarding for me during the making of this thesis. When I program audiovisual computational devices to react to sensors, I often find myself having to look at streams of data, having to program statistical methods to detect trends, such as averages, and then having to program a hard coded value on the software, and having to write a manual for setup or fine tuning for collaborators to be able to run it again under different conditions.
...
By \emph{instruments} I mean devices that act as transducers across mediums, processing an input, which results in an output.
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.75\linewidth,height=0.40\textheight,keepaspectratio]{images/sonic-youth-guitar.jpg}
